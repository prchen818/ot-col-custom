// file: processor/tailsamplingprocessor/internal/rats/encoder.go
...
func (e *BFSEncoder) Encode(trace ptrace.Traces) (typeID string, isAbnormal bool) {
	...
	for len(queue) > 0 {
		curID := queue[0]
		queue = queue[1:]
		path = append(path, labels[curID])

		if childIDs, ok := childrenMap[curID]; ok && len(childIDs) > 0 {
			// 对每个节点的子节点按字典序（label）排序
			sort.Slice(childIDs, func(i, j int) bool {
				li := labels[childIDs[i]]
				lj := labels[childIDs[j]]
				return li < lj
			})
